<template>
  <el-select @change="change" class="t-select" popper-class="p-select-table" ref="pSelect" v-bind="attrs"
    v-on="$listeners" :value-key="returnObj? configure.keywords.value :''">
    <!-- 表头 -->
    <div class="p-table-header" :class="{ 'fixed_head': fixedHead }">
      <table cellspacing="0" cellpadding="0" border="0" class="p-table-wrapper" style="width: 100%;">
        <thead>
  ...
  methods: {
    ...
    setPadding() {
      if (this.isShowPagination) {
        this.$refs.pSelect.$el.getElementsByClassName('el-select-dropdown__wrap')[0].style.paddingBottom = '50px'
      } else {
        this.$refs.pSelect.$el.getElementsByClassName('el-select-dropdown__wrap')[0].style.paddingBottom = '20px'
      }
    },
    // 计算最大高度
    setMaxHeight() {
      if (this.maxHeight) {
        this.$refs.pSelect.$el.getElementsByClassName('el-select-dropdown__wrap')[0].style.maxHeight = this.maxHeight
      }
      if (this.maxWidth) {
        this.$refs.pSelect.$el.getElementsByClassName('el-select-dropdown__wrap')[0].style.maxWidth = this.maxWidth
      }
    },
    // 使 input 获取焦点
    focus() {
      this.$refs.pSelect.focus()
    },
    // 使 input 失去焦点，并隐藏下拉框
    blur() {
      this.$refs.pSelect.blur()
    }
  }
