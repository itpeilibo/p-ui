<template>
  <el-select
    ...
    :class="{'p_select_table_tag_del':useVirtual&&multiple}"
    popper-class="p-select-table"
    :multiple="multiple"
  ...
  >
    <template #empty>
      <div class="p-table-select__table" :style="{ width: `${tableWidth}px` }">
        <div class="table_query_condition" v-if="isShowQuery">
          <p-query-condition ref="p-query-condition" v-bind="$attrs" v-on="$listeners">
...
export default {
  ...
  mounted() {
    ...
    // 获取查询条件组件的项
    this.$refs['p-query-condition'] && Object.values(this.$refs['p-query-condition'].opts).map(val => {
