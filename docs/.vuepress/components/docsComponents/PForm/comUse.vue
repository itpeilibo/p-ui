<template>
  <p-layout-page>
    <p-layout-page-item>
      <p-form :ref-obj.sync="formOpts.ref" :formOpts="formOpts" :widthSize="1" />
    </p-layout-page-item>
  </p-layout-page>
</template>
  ...
  data() {
    return {
      ...
      formOpts: {
        ...
        fieldList: [
          {
            label: '性别', value: 'sex', placeholder: 'PSelect单选', comp: 'p-select', isSelfCom: true, bind: { optionSource: [] }, eventHandle: {
              change: (val, scope) => this.changeSex(val, scope),
            }
          },
          { label: '状态(继承list)', value: 'status1', placeholder: 'PSelect继承list方式', type: 'select-arr', comp: 'p-select', list: 'statusList', arrLabel: 'label', arrKey: 'key', bind: {} },
          { label: '状态', value: 'status', placeholder: 'PSelect单选', comp: 'p-select', isSelfCom: true, bind: { optionSource: [] } },
          { label: '爱好', value: 'hobby', placeholder: 'PSelect多选', comp: 'p-select', isSelfCom: true, bind: { multiple: true, optionSource: [] } },
          { label: '部门', value: 'deptCode', placeholder: 'PCascader使用', comp: 'p-cascader', isSelfCom: true, bind: { props: { emitPath: false, children: 'children', label: 'deptName', value: 'deptNum', checkStrictly: true, }, options: [] } },
          ...
          {
            label: '下拉选择表格', value: 'deptCode2', placeholder: 'p-select-table使用', comp: 'p-select-table', isSelfCom: true, bind: {
              isKeyup: true,
              maxHeight: 400,
          ...
          { label: '年份', value: 'createDate', placeholder: 'PDatePicker选择年份', type: 'year', bind: { type: 'year' }, comp: 'p-date-picker' },
          { label: '月份范围', value: 'valDate1', comp: 'p-date-picker', bind: { type: 'monthrange', isPickerOptions: true } },
          { label: '日期范围', value: 'valDate2', comp: 'p-date-picker', bind: { type: 'daterange', isPickerOptions: true } },
          { label: '时间范围', value: 'valDate3', comp: 'p-date-picker', bind: { type: 'datetimerange', isPickerOptions: true } },
